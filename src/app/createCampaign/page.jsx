...
export default function CreateCampaignPage() {
  ...
  useEffect(() => {
    async function loadUserAndNGO() {
      ...
      if (currentUser?.profile?.role === 'ngo_admin') {
        setIsNGOAdmin(true)
        
        // Try to load their NGO
        const { ngo: userNGO } = await getMyNGO()
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            {isNGOAdmin ? (
              ...
              <>Have a cause you're passionate about? Start your own fundraising campaign!</>
            )}
          </p>
        </div>
        ...
        {!isNGOAdmin && (
          <div className="bg-blue-50 border-l-4 border-blue-500 p-6 rounded-lg mb-8">
            <div className="flex gap-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-900 mb-2">Anyone can start a campaign!</h3>
                <p className="text-blue-800 text-sm">
                  You don't need to be an NGO to make a difference. Start a campaign for any cause you care about. 
                  Our team will review and verify your campaign before it goes live.
                </p>
              </div>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-8 space-y-6">
          ...
          <div>
            <label htmlFor="campaignStory" className="block text-sm font-medium text-gray-700 mb-2">
          ...
          {!isNGOAdmin && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <h3 className="text-sm font-semibold text-yellow-900 mb-2">📋 Campaign Guidelines:</h3>
              <ul className="text-sm text-yellow-800 space-y-1 ml-4 list-disc">
                <li>Your campaign will be reviewed by our admin team (24-48 hours)</li>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {loading ? (
                ...
                isNGOAdmin ? 'Create Campaign' : 'Submit for Review'
              )}
            </button>
            <Link
              href={isNGOAdmin ? '/users/ngo/dashboard' : '/'}
          ...
          <div className="flex items-center justify-center gap-2 text-sm text-gray-600">
            {isNGOAdmin ? (
              ...
              <span className="inline-flex items-center gap-2 bg-purple-100 text-purple-700 px-4 py-2 rounded-full">
                ...
                Independent Campaign by {user?.profile?.name}
              </span>
            )}
          </div>
        </form>
