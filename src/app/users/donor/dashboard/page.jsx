...
import { getDonationHistory, getTotalDonated } from '@/app/actions/donation.actions'
// import { getMonthlySubscriptions } from '@/app/actions/monthly-support.actions'
...
export default async function DonorDashboard() {
  ...
  const { data : donations } = await getDonationHistory()
  console.log(donations)
  const { data: totalDonated } = await getTotalDonated()
//   const { subscriptions: monthlySubscriptions } = await getMonthlySubscriptions()
...
//     (sum, sub) => sum + Number(sub.monthly_donations_amt), 
//     0
//   )
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{followedNGOs.length}</p>
          ...
          {/* <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-orange-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">Monthly Support</p>
                <p className="text-3xl font-bold text-gray-900">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold">
                            {donation.ngos?.name || donation.campaigns?.title}
                          </p>
                          <p className="text-sm text-gray-500">
          ...
          <div className="space-y-6">
              ...
              {followedNGOs.length > 0 ? (
                <div className="space-y-3">
                  {followedNGOs.slice(0, 5).map((ngo) => (
                    ...
                    >
                      <div className="w-12 h-12 rounded-full bg-blue-600 text-white overflow-hidden flex items-center justify-center">
                        {ngo.name?.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold truncate">{ngo.name}</p>
                        <p className="text-sm text-gray-500">
                          {ngo.followers_count} followers
                        </p>
                      </div>
                    </Link>
                  ))}
            ...
            {monthlySubscriptions.length > 0 && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h2 className="text-xl font-bold mb-4">Monthly Support</h2>
                <div className="space-y-3">
                  {monthlySubscriptions.map((sub) => (
                    <div key={sub.id} className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                      <p className="font-semibold text-gray-900">{sub.ngos?.name}</p>
                      <p className="text-sm text-orange-700 font-bold">
                        ₹{Number(sub.monthly_donations_amt).toLocaleString()}/month
                      </p>
                    </div>
                  ))}
                </div>
